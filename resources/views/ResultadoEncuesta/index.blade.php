...
<a href="{{route('indexController.index')}}" class="btn btn-primary mr-2 text-white">
    ...
    Atrás</a>
<a href="{{route('encuesta.create')}}" class="btn btn-primary mr-2 text-white">
    <svg class="icono" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<h2 class="text-center mb-2">Administrar información encuesta</h2>
...
<div class="col-md-10 mx-auto bg-white p-3 mt-5">
    <table class="table table-hover" style="vertical-align: middle">
        ...
        <tbody>
            ...
            <tr>
                <th scole="col">{{$encuesta->created_at}}</th>
                <th scole="col">{{$encuesta->name}}</th>
                <th scole="col">{{$encuesta->tipo}}</th>
                <th scole="col">{{$encuesta->id}}</th>
                <th scole="col"> <a href="/storage/{{$encuesta->url}}">Ver adjunto</a></th>
                <th scole="col">
                    <a href="{{route('encuesta.show', ['id'=>$encuesta->id])}}" class="btn btn-outline-success  d-block mb-2">
                        <svg class="icono" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                        Ver</a>
                    <a href="{{route('encuesta.edit', ['id'=>$encuesta->id])}}" class="btn btn-outline-dark  d-block mb-2">
                        <svg class="icono" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                        Editar</a>

                    <eliminar-encuesta hoja-id={{$encuesta->id}}></eliminar-encuesta>
                </th>
            </tr>
            @endforeach
        </tbody>
