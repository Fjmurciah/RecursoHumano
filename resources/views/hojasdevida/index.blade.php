...
@section('botones')
    ...
    <a href="{{route('indexController.index')}}" class="btn btn-primary mr-2 text-white">
        ...
        Atrás</a>
    <a href="{{route('hojasdevida.create')}}" class="btn btn-primary mr-2 text-white">
        <svg class="icono" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        Crear hoja de vida</a>

@endsection
@section('content')

    <h2 class="text-center mb-2">Administrar hojas de vida</h2>
    ...
    <div class="col-md-10 mx-auto bg-white p-3 mt-5">
        <table class="table table-hover" style="vertical-align: middle">
            ...
            <tbody>
                @foreach ($hojasdevida as $hojas)
                    <tr>
                        <th scole="col">{{$hojas->name}}</th>
                        <th scole="col">{{$hojas->rol}}</th>
                        <th scole="col">{{$hojas->email}}</th>
                        <th scole="col">{{$hojas->estado}}</th>
                        <th scole="col">
                            <a href="{{route('hojasdevida.show', ['id'=>$hojas->id])}}" class="btn btn-outline-success  d-block mb-2">
                                <svg class="icono" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                  </svg>
                                Ver</a>
                            <a href="{{route('hojasdevida.edit', ['id'=>$hojas->id])}}" class="btn btn-outline-dark  d-block mb-2">
                                <svg class="icono" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
